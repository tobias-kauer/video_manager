...
class MockMosfet:
    def __init__(self, gpio_pin=17, frequency=1500):
        self.value = 0  # Simulate the brightness value
        self.is_running = False
        self.interrupt = False
    ...
    def blink(self, on_time=2, off_time=2):
        ...
        Args:
            ...
            off_time (int): Duration in seconds for the MOSFET to stay off.
        """
        print(f"[Mock] Starting blink with on_time={on_time} and off_time={off_time}.")
        self.is_running = True
        self.interrupt = False

        while not self.interrupt:
            self.value = 1  # Simulate turning on the MOSFET
            print("[Mock] MOSFET ON")
            time.sleep(on_time)

            if self.interrupt:  # Check if interrupted after turning on
                break

            self.value = 0  # Simulate turning off the MOSFET
            print("[Mock] MOSFET OFF")
            time.sleep(off_time)

        self.value = 0  # Ensure the MOSFET is turned off after interruption
        self.is_running = False
        print("[Mock] Blink operation interrupted or completed.")
    ...
    def pulse_smooth_with_range(self, duration=10, steps=100, min_brightness=0.2, max_brightness=0.8):
        ...
        print(f"[Mock] Starting smooth pulse for {duration} seconds with {steps} steps between {min_brightness} and {max_brightness}.")
        ...
        while not self.interrupt:
            ...
            for brightness in brightness_values:
                if self.interrupt:
                    print("[Mock] Pulse interrupted.")
                    break

                self.value = brightness  # Simulate setting the brightness
                print(f"[Mock] MOSFET brightness: {brightness:.2f}")
                time.sleep(step_duration)

        self.value = 0  # Ensure the MOSFET returns to a stable state
        self.is_running = False
        print("[Mock] Smooth pulse operation interrupted or completed.")
    ...
    def set_pwm(self, percentage):
        ...
        if not (0 <= percentage <= 100):
            raise ValueError("Percentage must be between 0 and 100.")
        self.value = percentage / 100  # Simulate setting the brightness
        print(f"[Mock] MOSFET brightness set to {percentage}%.")
    ...
    def interrupt_task(self):
        ...
        """
        self.interrupt = True
        print("[Mock] Interrupt signal sent.")